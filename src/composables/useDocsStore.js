...
// ---------------------------------------------------------------------------
// sessionStorage 恢复：模块内第一次使用 composable 时执行一次
// ---------------------------------------------------------------------------
function hydrate() {
  if (hydrated) return
  hydrated = true
...
export async function addZipFile(file) {
  let result
  try {
...
export function getTabContext(key) {
  ...
  if (!t) return { isBuiltin: false, title: '', content: '', loading: false, error: 'tab 已关闭', imageResolver: null }
  return {
    isBuiltin: false,
    title: t.title,
    content: t.content,
...
// ---------------------------------------------------------------------------
// 模块侧效应：导入即恢复 sessionStorage 快照 + 挂 push/splice 自动 persist 的 watcher
// 项目无 SSR，sessionStorage 可用；watcher 与模块同生命期，无需手动 stop
// ---------------------------------------------------------------------------
hydrate()
watch(
